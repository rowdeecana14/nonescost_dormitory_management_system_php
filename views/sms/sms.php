...
<head>
	...
	.skin-blue .main-header .navbar {
		...
		border-bottom: 2px solid #21f956;
	}
	.top-title {
		width: 90%;
		min-height: 20px;
	  			...
	  			<div style="margin: 5px 40px;">
	                ...
	                <div id="message" style="margin-bottom: 70px;">
	                	<!-- <div class="alert alert-success text-center" role="alert">
                          <strong><span class="fa fa-send"></span> SMS was succeffully send.</strong>
                        </div> -->
                    </div>
	  			</div>
        ...
        if(str.length == 0){
            ...
            try {
              ...
              if(response.is_exist == true) {
              	...
              	$("#profile").attr("src", "../../images/"+response.data.image);
              	$("#name").text(response.data.fname + " " + response.data.mname + " " + response.data.lname);
              	$("#barcode").attr('disabled','disabled');

              	setTimeout(function() {
              		console.log("go send the message")
              		query.path = '../../controllers/sms.php';
	                query.param = {
	                    action: 'send',
	                    input: response.data.contact,
